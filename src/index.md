...
@zhaoConditionalVariationalAutoencoder2023 propose CV-SLT, an encoder-decoder based approach using conditional variational autoencoders. In a nutshell, they run the video data through once with text and once without, and the model must learn how to correct for this. They argue that the modality gap between video and text is a major issue, using the MMTLB method from [@chenSimpleMultiModalityTransfer2022a] to embed videos and text from PHOENIX2014T and quantify the modality gap, then showing that more similar embeddings tend to trend with better BLEU scores. Their method to deal with this is a framework for training with two paths, which they call "prior" and "posterior". In the first the model encodes only visual information to predict text (to help regularize the encoder), and in the second the model uses both visual and text information (to help regularize the decoder). They also use something they call Attention Residual Gaussian Distribution (ARGD) when integrating the text to the "posterior" path, which "considers the textual information as a residual component relative to the prior distribution", which presumably means that the posterior track can use the prior track's distribution as a starting point and the text information as a correction. The result of all of this is a method that requires only video and text for training, and no gloss supervision. They evaluate on PHOENIX2014T [@cihan2018neural] and CSL-Daily [@dataset:huang2018video]. They provide [code](https://github.com/rzhao-zhsq/CV-SLT) implementation, which according to them is largely based on [the implementation of SLRT](https://github.com/FangyunWei/SLRT) from [@chenSimpleMultiModalityTransfer2022a].
